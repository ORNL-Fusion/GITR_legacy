...
// show memory usage of GPU
  ...
  if (world_rank == 0) {
    importVectorField(cfg, input_path, BFIELD_INTERP, bfieldCfg, nR_Bfield, nY_Bfield, nZ_Bfield, bfieldGridr.front(), bfieldGridy.front(), bfieldGridz.front(), br.front(), by.front(), bz.front(), bfieldFile);
  }
#if USE_MPI > 0
  ...
  std::cout << "node " << world_rank << "Bfield at 5.5 -4 " << Btest[0] << " " << Btest[1] << " " << Btest[2] << std::endl;
  ...
  MPI_Type_create_struct(nBoundaryMembers, lengths, offsets, types, &boundary_type);
...
//}
  ...
  if (world_rank == 0) {
    for (int i = 0; i < nHashes; i++) {
      ...
      if (i > 0)
        ncIndex = nR_closeGeom[i - 1] * nY_closeGeom[i - 1] * nZ_closeGeom[i - 1] * n_closeGeomElements[i - 1];
...
#endif
      if (i > 0)
        dataIndex = nR_closeGeom[0] * nY_closeGeom[0] * nZ_closeGeom[0] * n_closeGeomElements[0];
...
//                               ompPrint());
  ...
  for (int i = 0; i < nSourceElements; i++) {
    ...
    std::cout << "particleSourceSpaceCDF " << i << " " << particleSourceIndices[i] << " " << particleSourceX[i] << " " << particleSourceZ[i] << particleSourceSpaceCDF[i] << std::endl;
  }
