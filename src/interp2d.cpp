...
float interp1dUnstructured(float samplePoint, int nx, float max_x, float *data, int &lowInd) {
  ...
  //  std::endl;
  interpolated_value =
      ((data[low_index + 1] - samplePoint) * low_index * max_x / nx + (samplePoint - data[low_index]) * (low_index + 1) * max_x / nx) / (data[low_index + 1] - data[low_index]);
...
float interp1dUnstructured2(float samplePoint, int nx, float *xdata, float *data) {
  ...
  interpolated_value = xdata[low_index] + (samplePoint - data[low_index]) * (xdata[low_index + 1] - xdata[low_index]) / (data[low_index + 1] - data[low_index]);
