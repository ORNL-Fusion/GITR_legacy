  ...
  for (int i = 0; i < nX; i++) {
    for (int j = 0; j < nY; j++) {
      for (int k = 0; k < nZ; k++) {
        ...
        if (std::isnan(spline) || std::isinf(spline))
          spline = 0.0;
...
int importGeometry(libconfig::Config &cfg_geom, sim::Array<Boundary> &boundaries) {
  ...
  for (int i = 0; i < nLines; i++) {
    ...
    outfile << "geom(" << i + 1 << ",:) = [" << boundaries[i].x1 << ", " << boundaries[i].z1 << ", " << boundaries[i].x2 << ", " << boundaries[i].z2 << ", " << boundaries[i].slope_dzdx << ", " << boundaries[i].intercept_z << ", " << boundaries[i].length << ", " << boundaries[i].Z << "];" << std::endl;
