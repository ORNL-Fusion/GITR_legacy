...
float rateCoeffInterp(int charge, float te, float ne, int nT, int nD, float *rateGrid_Tempp, float *rateGrid_Densp, float *Ratesp) {
  ...
  */
  ...
  float fx_z1 = (aN * std::pow(10.0f, Ratesp[charge * nT * nD + indT * nD + indN]) + bN * std::pow(10.0f, Ratesp[charge * nT * nD + indT * nD + indN + 1])) / abN;

  float fx_z2 = (aN * std::pow(10.0f, Ratesp[charge * nT * nD + (indT + 1) * nD + indN]) + bN * std::pow(10.0f, Ratesp[charge * nT * nD + (indT + 1) * nD + indN + 1])) / abN;
