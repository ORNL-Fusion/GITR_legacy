...
struct geometry_check {
  ...
  void operator()(std::size_t indx) const {
    ...
    if (particlesPointer->hitWall[indx] == 0.0) {
      ...
      {
        float pi = 3.14159265;
        float theta = std::atan2(particlesPointer->y[indx], particlesPointer->x[indx]);
        float thetaPrev = std::atan2(particlesPointer->yprevious[indx], particlesPointer->xprevious[indx]);
        //float vtheta = std::atan2(particlesPointer->vy[indx],particlesPointer->vx[indx]);
        float rprev = std::sqrt(particlesPointer->xprevious[indx] * particlesPointer->xprevious[indx] +
        ...
        if (theta <= boundaryVector[nLines].y1) {
          particlesPointer->xprevious[indx] = r * std::cos(boundaryVector[nLines].y2 + theta);
          particlesPointer->yprevious[indx] = r * std::sin(boundaryVector[nLines].y2 + theta);
          particlesPointer->x[indx] = rprev * std::cos(boundaryVector[nLines].y2 + theta);
          particlesPointer->y[indx] = rprev * std::sin(boundaryVector[nLines].y2 + theta);

          vx0 = vrMag * std::cos(boundaryVector[nLines].y2 + theta) - vthetaMag * std::sin(boundaryVector[nLines].y2 + theta);
          vy0 = vrMag * std::sin(boundaryVector[nLines].y2 + theta) + vthetaMag * std::cos(boundaryVector[nLines].y2 + theta);
          particlesPointer->vx[indx] = vx0;
          particlesPointer->vy[indx] = vy0;
        } else if (theta >= boundaryVector[nLines].y2) {
          particlesPointer->xprevious[indx] = rprev * std::cos(thetaPrev - boundaryVector[nLines].y2);
          particlesPointer->yprevious[indx] = rprev * std::sin(thetaPrev - boundaryVector[nLines].y2);
          particlesPointer->x[indx] = r * std::cos(theta - boundaryVector[nLines].y2);
          particlesPointer->y[indx] = r * std::sin(theta - boundaryVector[nLines].y2);

          vx0 = vrMag * std::cos(theta - boundaryVector[nLines].y2) - vthetaMag * std::sin(theta - boundaryVector[nLines].y2);
          vy0 = vrMag * std::sin(theta - boundaryVector[nLines].y2) + vthetaMag * std::cos(theta - boundaryVector[nLines].y2);
          particlesPointer->vx[indx] = vx0;
          particlesPointer->vy[indx] = vy0;
        }
      }
      ...
      int zInd = std::floor((particlesPointer->zprevious[indx] - closeGeomGridz[zHashInd]) / dz + 0.5f);
      int i = 0;
      int yInd = std::floor((particlesPointer->yprevious[indx] - closeGeomGridy[yHashInd]) / dy + 0.5f);
...
//std::cout << "WARNING: particle outside of geometry hash range (low)" << std::endl;
      ...
      int buffIndx = 0;
      if (nHash > 0)
        buffIndx = nR_closeGeom[nHash - 1] * nY_closeGeom[nHash - 1] * nZ_closeGeom[nHash - 1] * n_closeGeomElements[nHash - 1];
      //std::cout << "buff Index " << buffIndx << std::endl;
      for (int j = 0; j < n_closeGeomElements[nHash]; j++) {
        i = closeGeom[buffIndx + zInd * nY_closeGeom[nHash] * nR_closeGeom[nHash] * n_closeGeomElements[nHash] + yInd * nR_closeGeom[nHash] * n_closeGeomElements[nHash] + rInd * n_closeGeomElements[nHash] + j];
      ...
      for (int i = 0; i < nLines; i++) {
        ...
        if (signPoint0 != signPoint1) {
          ...
          if (hitSurface == 1) {
            ...
            //  #endif
            //#endif
            float E0 = 0.5 * particlesPointer->amu[indx] * 1.66e-27 * (particlesPointer->vx[indx] * particlesPointer->vx[indx] +
...
//float thetaImpact = std::acos(partDotNormal)*180.0/3.1415;
//if(E0 < surfaces->E && E0> surfaces->E0)
//{
//    int tally_index = std::floor((E0-surfaces->E0)/surfaces->dE);
//    if(thetaImpact > surfaces->A0 && thetaImpact < surfaces->A)
//    {
//        int aTally = std::floor((thetaImpact-surfaces->A0)/surfaces->dA);
...
#else //2D geometry
#if USECYLSYMM > 0
      float pdim1 = std::sqrt(particlesPointer->x[indx] * particlesPointer->x[indx] + particlesPointer->y[indx] * particlesPointer->y[indx]);
      float pdim1previous = std::sqrt(particlesPointer->xprevious[indx] * particlesPointer->xprevious[indx] + particlesPointer->yprevious[indx] * particlesPointer->yprevious[indx]);
      float theta0 = std::atan2(particlesPointer->yprevious[indx], particlesPointer->xprevious[indx]);
      float theta1 = std::atan2(particlesPointer->y[indx], particlesPointer->x[indx]);
      float thetaNew = 0;
      float rNew = 0;
      float xNew = 0;
      float yNew = 0;
...
//std::cout << "r0 " << particlesPointer->x[indx]previous << " " << particlesPointer->y[indx]previous << " " << particlesPointer->z[indx]previous<< std::endl;
#if GEOM_HASH > 0
#if USECYLSYMM > 0
      float r_position = std::sqrt(particlesPointer->xprevious[indx] * particlesPointer->xprevious[indx] + particlesPointer->yprevious[indx] * particlesPointer->yprevious[indx]);
#else
      float r_position = particlesPointer->xprevious[indx];
#endif
      ...
      int zInd = std::floor((particlesPointer->zprevious[indx] - closeGeomGridz[0]) / dz + 0.5f);
      ...
      for (int i = 0; i < nLines; i++) {
#endif
        //std::cout << "vert geom " << i << "  " << std::abs(boundaryVector[i].slope_dzdx) << " " << tol << std::endl;
        ...
        if (signPoint != signPoint0) {
          ...
          if (signLine1 != signLine2) {
            ...
            //std::cout << std::abs(particlesPointer->x[indx] - particlesPointer->xprevious[indx]) << tol_small << std::endl;
            if (std::abs(pdim1 - pdim1previous) < tol_small) {
              ...
              intersectiony[nIntersections - 1] = intersectionx[nIntersections - 1] * particle_slope + particle_intercept;
            }
          }
        }
      }
...
//std::cout << "yprev,intersectiony,zprevious,z,y " << particlesPointer->yprevious[indx] << " " << intersectiony[0] << " "<< particlesPointer->zprevious[indx] << " " << particlesPointer->z[indx] << " " << particlesPointer->y[indx] << std::endl;
          ...
          particlesPointer->yprevious[indx] = yNew;
          particlesPointer->y[indx] = yNew;
//float rrr  = std::sqrt(particlesPointer->x[indx]*particlesPointer->x[indx] + particlesPointer->y[indx]*particlesPointer->y[indx]);
...
//std::cout << "yprev,intersectiony,zprevious,z,y " << particlesPointer->yprevious[indx] << " " << intersectiony[0] << " "<< particlesPointer->zprevious[indx] << " " << particlesPointer->z[indx] << " " << particlesPointer->y[indx] << std::endl;
        ...
        particlesPointer->yprevious[indx] = intersectionx[minDistInd] * std::sin(thetaNew);
        particlesPointer->y[indx] = particlesPointer->yprevious[indx];
        //particlesPointer->y[indx] = intersectionx[minDistInd]*std::cos(thetaNew);
        particlesPointer->x[indx] = intersectionx[minDistInd] * std::cos(thetaNew);
#else
