...
float getE(float x0, float y, float z, float E[], Boundary *boundaryVector, int nLines,
  ...
  {
    i = closeGeom[zInd * nY_closeGeom * nR_closeGeom * n_closeGeomElements + yInd * nR_closeGeom * n_closeGeomElements + rInd * n_closeGeomElements + k];
...
#else
  for (int j = 0; j < nLines; j++) { //std::cout << " surface check " << j << std::endl;
#endif
    //if(j > nLines)
    //{
    //    j = 0;
