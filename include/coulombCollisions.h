...
void getSlowDownFrequencies(float &nu_friction, float &nu_deflection, float &nu_parallel,
  ...
  gam_ion_background = 0.238762895 * std::pow(charge, 2) * std::pow(background_Z, 2) * std::log(lam) / (amu * amu); //constant = Q^4/(MI^2*4*pi*EPS0^2)
                                                                                                                    //std::cout << "gam components " <<gam_electron_background << " " << std::pow(Q,4) << " " << " " << std::pow(background_Z,2) << " " << log(lam)<< std::endl;
  ...
  //psi_prime = 2*std::sqrt(xx/pi)*expf(-xx);
  //psi_psiprime = erf(1.0*std::sqrt(xx));
  ...
  //psi_prime_e = 2*std::sqrt(xx_e/pi)*expf(-xx_e);
  //psi_psiprime_e = erf(1.0*std::sqrt(xx_e));
  ...
  //    	    std::cout << "gam_electron_background*density/std::pow(velocityNorm,3) " << gam_electron_background<< " " << density<< " " << velocityNorm << " " << lam_d<< std::endl;
  //    	    }
...
#if PARTICLESEEDS > 0
#ifdef __CUDACC__
      //int plus_minus1 = 1;//std::floor(curand_uniform(&particlesPointer->streams_collision1[indx]) + 0.5)*2 -1;
      //int plus_minus2 = 1;//std::floor(curand_uniform(&particlesPointer->streams_collision2[indx]) + 0.5)*2 -1;
      //int plus_minus3 = 1;//std::floor(curand_uniform(&particlesPointer->streams_collision3[indx]) + 0.5)*2 -1;
...
//particlesPointer->test[indx] = xsi;
      ...
      //int plus_minus1 = std::floor(dist(state[indx]) + 0.5)*2 - 1;
      //int plus_minus2 = std::floor(dist(state[indx]) + 0.5)*2 - 1;
      //int plus_minus3 = std::floor(dist(state[indx]) + 0.5)*2 - 1;
...
#else
#if __CUDACC__
      //float plus_minus1 = std::floor(curand_uniform(&state[3]) + 0.5)*2-1;
      //float plus_minus2 = std::floor(curand_uniform(&state[4]) + 0.5)*2-1;
      //float plus_minus3 = std::floor(curand_uniform(&state[5]) + 0.5)*2-1;
      float n2 = curand_normal(&state[indx]);
      float xsi = curand_uniform(&state[indx]);
#else
      std::normal_distribution<double> distribution(0.0, 1.0);
      ...
      float n2 = distribution(state[indx]);
      float xsi = dist(state[indx]);
//float plus_minus1 = std::floor(dist(state[3]) + 0.5)*2 - 1;
//float plus_minus2 = std::floor(dist(state[4]) + 0.5)*2 - 1;
//float plus_minus3 = std::floor(dist(state[5]) + 0.5)*2 - 1;
#endif
#endif
      ...
      //        float vxy00 = std::sqrt(vTherm*vTherm - vz*vz);
      //        float vxy01 = std::sqrt(vx*vx + vy*vy);
      ...
      //std::cout << "nu_paralel base " << std::sqrt(0.5*nu_parallel*dt*vRel2) << std::endl;
      ...
      //SFT
      //coeff_par = n1*std::sqrt(dt/tau_par);
      //int plumin3 = 2*std::floor(r3+0.5) - 1;
      //float coeff_par = plumin3*std::sqrt(0.5*std::abs(nu_energy)*dt*vRel2);
      //float coeff_par = 1.0;//+ n1*std::sqrt(nu_parallel*dt);
      //float coeff_par = 1.0 - nu_friction*dt;
      //int plumin1 = 2*std::floor(r1+0.5) - 1;
      //int plumin2 = 2*std::floor(r2+0.5) - 1;
      float cosXsi = std::cos(2.0 * pi * xsi);
      float sinXsi = std::sin(2.0 * pi * xsi);
...
//std::cout << "cosXsi and sinXsi " << cosXsi << " " << sinXsi << std::endl;
      ...
      //vPartNorm = std::sqrt(vfx*vfx + vfy*vfy + vfz*vfz);
      //float vzNew = vz + drag*std::copysign(1.0,parallel_direction[2]);///velocityCollisionsNorm;

      //float vxy0 = std::sqrt(vx*vx + vy*vy);
      //float vxy = std::sqrt(vPartNorm*vPartNorm - vzNew*vzNew);
      ////std::cout << "vzNew vxy0 vxy " << vzNew << " " << vxy0 << " " << vxy << endl;
